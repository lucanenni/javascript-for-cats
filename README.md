*Note: this repository is an unmodified fork of [maxogden/javascript-for-cats](https://github.com/maxogden/javascript-for-cats), kept here only as the reference source for the Italian translation in [javascript-for-cats-it](https://github.com/lucanenni/javascript-for-cats-it). It isn't maintained by this account, so the tutorial content and its licensing are entirely up to the upstream project — please direct any content questions, issues, or contributions [there](https://github.com/maxogden/javascript-for-cats/issues). The only changes made in this fork are non-content maintenance, done on 2026-08-14: removed the dead Google Analytics (Universal Analytics, sunset by Google in 2023) tracking snippet; patched the build dependencies (`marked`, `mustache`) for known high-severity security advisories; replaced the abandoned, already-broken `gh-pages-deploy` tool with the maintained `gh-pages` package; and removed `javascripts/` (rainbow.js and friends), which were never actually loaded by the page. Deploys are automatic: a GitHub Action renders the site on every push to `master` and publishes it straight through GitHub's official Pages deployment (no `gh-pages` branch, no extra git credentials involved) — it deliberately excludes the `CNAME` file, since `jsforcats.com` is the upstream project's domain, not this fork's.*
